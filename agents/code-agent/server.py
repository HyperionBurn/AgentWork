"""
AgentWork — Code Agent
Code generation, implementation, and refactoring specialist.

x402 paywalled endpoint: /api/generate
"""
...
AGENT_VERSION = "1.0.0"
AGENT_PORT = int(os.getenv("CODE_AGENT_PORT", "4022"))
AGENT_DESCRIPTION = "Code generation, implementation, and refactoring specialist"
SELLER_ADDRESS = os.getenv("CODE_AGENT_WALLET", "")
...
AGENT_PRICE = normalize_price("CODE_AGENT_PRICE", "$0.005")
...
try:
    ...
    if SELLER_ADDRESS:
        ...
        logger.warning(
            "CODE_AGENT_WALLET not set — running in passthrough mode (no payments)"
        )
...
def index():
    return jsonify({
        ...
        "pricing": {"/api/generate": AGENT_PRICE},
...
def generate():
    ...
    logger.info(f"Code generation request: {task}" + (f" [context: {context[:50]}...]" if context else ""))

    result = perform_code_generation(task, context)
